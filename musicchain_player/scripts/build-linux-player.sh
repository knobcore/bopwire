#!/usr/bin/env bash
#
# Build the musicchain Linux player (Flutter desktop).
#
# Required env vars:
#   FLUTTER                Path to flutter binary
#   MC_LIB_DIR             Folder holding libmusicchain.so + libmc_rats.so
...
: "${MC_LIB_DIR:?MC_LIB_DIR not set — point at a dir holding libmusicchain.so + libmc_rats.so}"
