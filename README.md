...
| Tree | Description |
| --- | --- |
| `musicchain/` | C++ home + mini-node, leveldb-backed chain, swarm registry, fingerprint matching (chromaprint), librats peer transport. Vendored librats sits under `musicchain/deps/librats/`. |
| `musicchain_player/` | Cross-platform Flutter player. Android, Windows, Linux desktop. Streams audio from the swarm via a loopback HTTP proxy + media_kit. Foreground-service-pinned on Android so the librats client survives screen-off. |
| `scripts/` | One build script per shipping target — see below. |

## How the pieces fit

1. **Home node** (`musicchain/tools/node_main.cpp`) holds the chain in leveldb, mints blocks via a 5-min heartbeat producer, exposes RPC over librats (`musicchain.request` topic). Persists swarm membership so a restart doesn't wipe it.
2. **Mini node** (`musicchain/tools/mini_node.cpp`) — a tiny rendezvous on the VPS that relays peer addresses, forwards RPC + binary streams for peers behind symmetric NATs, and tracks player/full-node liveness.
...
| --- | --- | --- |
| Windows full node | `scripts\build-node-windows.ps1` | `musicchain\build-win64\Release\musicchain-node.exe` |
| Linux full node | `scripts/build-node-linux.sh` | `musicchain/build-linux/musicchain-node` |
| Linux mini node | `scripts/build-mini-node-linux.sh` | `musicchain/build-linux-mini/musicchain-mini-node` |
| Windows player | `scripts\build-player-windows.ps1` | `musicchain_player\build\windows\x64\runner\Release\` |
| Android APK | `scripts\build-player-android.ps1` | `musicchain_player\build\app\outputs\flutter-apk\app-release.apk` |
...
vcpkg is auto-cloned into `musicchain\vcpkg\` if it isn't already on `VCPKG_ROOT` or one of the common locations.
...
`build-player-android.ps1` expects the prebuilt OpenSSL and chromaprint `.so` files to already be staged under `musicchain_player/android/app/src/main/jniLibs/arm64-v8a/`. The OpenSSL build helper is at `musicchain/build_openssl_android.sh` (Linux/WSL) or `musicchain/build_openssl_android_win.sh` (Windows MSYS2). Chromaprint cross-compile artifacts live at `chromaprint-android-arm64/` in the repo root.

## Releases

Pre-built binaries live on the GitHub Releases page:

- **Windows home node** — `musicchain-node.exe` + DLLs.
- **Linux home node** — `musicchain-node` (Ubuntu 22.04 toolchain).
- **Linux mini node** — `musicchain-mini-node` (statically linked where possible).
- **Windows player** — `musicchain_player.exe` + DLLs.
