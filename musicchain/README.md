...
```bash
./musicchain-node start --data-dir ./data --api-port 9334 --p2p-port 9333
```
...
```
musicchain-node start [--config path] [--data-dir path] [--p2p-port n] [--api-port n]
musicchain-node status
musicchain-node peers
musicchain-node verify-chain [--data-dir path]
musicchain-node rebuild-index [--data-dir path]
```
...
```
musicchain/
  include/musicchain.h        Public C API (for Flutter FFI)
  src/
