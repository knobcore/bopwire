...
bool RatsClient::handle_handshake_message(socket_t socket, const std::string& initial_peer_id, const std::vector<uint8_t>& data) {
    ...
    {
        auto existing_it = peers_.find(handshake_msg.peer_id);
        if (existing_it != peers_.end()) {
            socket_t existing_sock = existing_it->second.socket;
            if (existing_sock == socket) {
                // Same socket already mapped to this peer_id; a duplicate
                // handshake frame arrived on the established connection.
                // Drop it without tearing down anything.
                LOG_CLIENT_INFO("Duplicate handshake on already-mapped socket for peer "
                                << handshake_msg.peer_id << ", ignoring");
                return false;
            }
            // A different socket holds the same peer_id. That entry is stale:
            // the previous TCP connection died (peer rebooted, NAT rebind,
            // watchdog redial faster than our cleanup tick) and we haven't
            // noticed yet. Evict it and let the freshly-handshook socket win,
            // otherwise the peer is locked out until our side observes the
            // half-open close — which can take minutes and produces an endless
            // dial->Peer disconnected storm.
            LOG_CLIENT_INFO("Peer " << handshake_msg.peer_id
                            << " has a stale entry on socket " << existing_sock
                            << "; evicting it in favor of new socket " << socket);
            remove_peer_by_id_unlocked(handshake_msg.peer_id);
            // poller_remove takes io_mutex_, distinct from peers_mutex_;
            // close_socket is just an OS-level close. Both are safe here.
            poller_remove(existing_sock);
            close_socket(existing_sock);
            // peer_it (located via the current socket under its old peer_id
            // key) is unaffected by the erase above and remains valid.
        }
    }
