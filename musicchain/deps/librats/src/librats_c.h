...
// looking for other holders. The peers-found callback is invoked
// asynchronously by the DHT thread every time librats receives a fresh
// batch of peer addresses.
//
// OWNERSHIP CONTRACT (musicchain patch — diverges from upstream
// librats which used transient pointers): every `const char*` in the
// `peer_addresses` array, AND the array itself, are heap-allocated
// before the callback fires. The consumer MUST free each entry and
// the array via `rats_string_free` (or plain `free()`), even on no-op
// consumption. Failing to free per-batch leaks ~32 bytes per
// peer address per delivery. The contract change makes the API safe
// for callers (e.g. Dart's NativeCallable.listener) that can't read
// the strings synchronously inside the callback.
