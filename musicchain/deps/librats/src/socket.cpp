...
#include <chrono>
#ifdef _WIN32
    // SIO_KEEPALIVE_VALS lives here, distinct from the Winsock2 baseline.
    #include <mstcpip.h>
#endif
#ifndef _WIN32
...
// Enable aggressive TCP keepalive so a half-open socket is detected within
// ~30 s instead of the OS default (~2 hours on Linux/Windows). Critical for
// mobile clients that flip wifi↔cellular: the old socket's peer can no
// longer reach us via the dropped network, but neither side observes the
// TCP close — keepalive probes force the dead path to surface so the
// reconnect watchdog can dial a fresh socket.
//
// idle_s:  seconds of inactivity before the first probe        (default 15)
// intvl_s: seconds between probes                              (default 5)
// count:   probes without ack before declaring the peer dead   (default 3)
//
// Net effect: dead socket noticed in idle_s + intvl_s*count ≈ 30 s.
static void apply_tcp_keepalive(socket_t sock,
                                int idle_s  = 15,
                                int intvl_s = 5,
                                int count   = 3) {
...
#ifdef _WIN32
    // Windows uses SIO_KEEPALIVE_VALS to set the idle + interval together.
    // The probe count is fixed by the OS (10 on modern Windows, but the
    // socket is RST'd well before that thanks to the tight interval).
    struct {
        unsigned long onoff;
        unsigned long keepalivetime;     // ms
        unsigned long keepaliveinterval; // ms
    } ka = { 1,
             static_cast<unsigned long>(idle_s)  * 1000UL,
             static_cast<unsigned long>(intvl_s) * 1000UL };
    DWORD bytes_returned = 0;
    if (WSAIoctl(sock, SIO_KEEPALIVE_VALS, &ka, sizeof(ka),
                 nullptr, 0, &bytes_returned, nullptr, nullptr) != 0) {
        LOG_SOCKET_WARN("SIO_KEEPALIVE_VALS failed on socket " << sock
                        << " err=" << WSAGetLastError());
    }
#else
  #ifdef TCP_KEEPIDLE
    ...
    // Belt-and-suspenders: TCP_USER_TIMEOUT (Linux 2.6.37+) drops the
    // connection when an outstanding write goes unacked for this long,
    // regardless of keepalive state. Useful when the app is actively
    // sending (RPC traffic) at the moment the network flips.
    int user_timeout_ms = (idle_s + intvl_s * count) * 1000;
