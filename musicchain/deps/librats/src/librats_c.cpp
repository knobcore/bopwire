...
void rats_set_connection_callback(rats_client_t handle, rats_connection_cb cb, void* user_data) {
    ...
    // musicchain patch: strdup peer_id so async consumers can keep the
    // pointer; sync consumers MUST rats_string_free it.
    wrap->client->set_connection_callback([wrap](socket_t, const std::string& peer_id) {
        if (wrap->connection_cb) {
            wrap->connection_cb(wrap->connection_ud, strdup(peer_id.c_str()));
        }
    });
}
...
rats_error_t rats_announce_for_hash(rats_client_t handle, const char* content_hash, int port,
    ...
    if (callback) {
        // OWNERSHIP CONTRACT: each peer address is strdup'd onto the
        // heap before the consumer callback runs, and the array of
        // pointers is also a heap allocation. The consumer is
        // responsible for calling rats_string_free on every entry AND
        // on the array — even on partial / no-op consumption.
        //
        // The original transient-pointer design was incompatible with
        // Dart's NativeCallable.listener pattern, which marshals the
        // call across a SendPort and dereferences the pointers
        // asynchronously after this lambda has already returned. C++
        // callers consume synchronously and just have to remember to
        // free; the contract is honest about the lifetime instead of
        // dangling.
        cpp_callback = [callback, user_data](const std::vector<std::string>& peers) {
            const size_t n = peers.size();
            // strdup'd pointer array. malloc rather than `new` so the
            // free side stays inside the C ABI (rats_string_free uses
            // free() under the hood). We deliberately tolerate the
            // pessimistic over-allocation when n==0 so the contract
            // stays "consumer always frees the array" rather than
            // having to nullcheck.
            const char** owned = static_cast<const char**>(
                std::malloc(sizeof(const char*) * (n == 0 ? 1 : n)));
            if (!owned) {
                // No heap; bail before invoking consumer with a null
                // array, which they wouldn't be able to free anyway.
                return;
            }
            for (size_t i = 0; i < n; ++i) {
                owned[i] = strdup(peers[i].c_str());
                if (!owned[i]) {
                    // strdup failed mid-batch — free what we already
                    // allocated and bail. Consumer never sees a half-
                    // populated array.
                    for (size_t j = 0; j < i; ++j) std::free((void*)owned[j]);
                    std::free(owned);
                    return;
                }
            }
            callback(user_data, owned, static_cast<int>(n));
        };
    }
...
rats_error_t rats_on_message(rats_client_t handle, const char* message_type, 
    ...
    // musicchain patch: strdup both args so async consumers (Dart's
    // NativeCallable.listener) can read them after the lambda returns.
    // Synchronous consumers MUST call rats_string_free on both.
    wrap->client->on(std::string(message_type), [wrap, callback, user_data](const std::string& peer_id, const nlohmann::json& data) {
        if (callback) {
            std::string json_str = data.dump();
            char* peer_copy = strdup(peer_id.c_str());
            char* data_copy = strdup(json_str.c_str());
            callback(user_data, peer_copy, data_copy);
        }
    });
...
void rats_set_binary_callback(rats_client_t handle, rats_binary_cb cb, void* user_data) {
    ...
    // musicchain patch: strdup/malloc the args so async consumers
    // (Dart NativeCallable.listener) can read them on a different
    // isolate event-loop tick. Synchronous consumers MUST call
    // rats_string_free on both pointers.
    wrap->client->set_binary_data_callback([wrap](socket_t, const std::string& peer_id, const std::vector<uint8_t>& data) {
        if (wrap->binary_cb) {
            char* peer_copy = strdup(peer_id.c_str());
            void* data_copy = std::malloc(data.size());
            if (data_copy) std::memcpy(data_copy, data.data(), data.size());
            wrap->binary_cb(wrap->binary_ud, peer_copy, data_copy, data.size());
        }
    });
}
