#!/usr/bin/env pwsh
<#
.SYNOPSIS
  Stop the home node if running, wipe its chain state, and re-start it.

.DESCRIPTION
  Wipes:
    - $DATA_DIR/blockchain.db   (leveldb)
    - $DATA_DIR/blocks          (.blk dump tree)
    - $DATA_DIR/logs            (old log files)
  Keeps:
    - $DATA_DIR/keys            (founder keypair survives so the chain
                                 can be re-bootstrapped under the same
                                 identity)

  Env vars:
    DATA_DIR      Default: C:\Users\lain\Music\musicchain
    NODE_EXE      Default: build-win64\Release\musicchain-node.exe
                  (resolved relative to the musicchain repo root)
    CONFIG_PATH   Default: build-win64\Release\full-node.config.json

  Usage:
    pwsh scripts\wipe-and-run-home.ps1
#>

$ErrorActionPreference = "Stop"
Set-StrictMode -Version Latest

$repoRoot   = (Resolve-Path "$PSScriptRoot\..").Path
$DATA_DIR   = if ($env:DATA_DIR)   { $env:DATA_DIR }   else { "C:\Users\lain\Music\musicchain" }
$NODE_EXE   = if ($env:NODE_EXE)   { $env:NODE_EXE }   else { Join-Path $repoRoot "build-win64\Release\musicchain-node.exe" }
$CONFIG_PATH= if ($env:CONFIG_PATH){ $env:CONFIG_PATH }else { Join-Path $repoRoot "build-win64\Release\full-node.config.json" }

if (-not (Test-Path $NODE_EXE)) {
    Write-Error "Node binary not found: $NODE_EXE"
}

# Stop any running musicchain-node so it releases the leveldb lock.
$running = Get-Process musicchain-node -ErrorAction SilentlyContinue
if ($running) {
    Write-Host "[wipe] stopping running musicchain-node (pid $($running.Id))"
    $running | Stop-Process -Force
    Start-Sleep -Seconds 2
}

Write-Host "[wipe] DATA_DIR=$DATA_DIR"
foreach ($sub in @("blockchain.db", "blocks", "logs")) {
    $p = Join-Path $DATA_DIR $sub
    if (Test-Path $p) {
        Write-Host "[wipe] rm -rf $p"
        Remove-Item -Recurse -Force $p
    }
}
New-Item -ItemType Directory -Force (Join-Path $DATA_DIR "blockchain.db") | Out-Null
New-Item -ItemType Directory -Force (Join-Path $DATA_DIR "blocks")        | Out-Null
New-Item -ItemType Directory -Force (Join-Path $DATA_DIR "logs")          | Out-Null

# Patch the config to point at the home data_dir (in case it was checked
# in with ./data).
if (Test-Path $CONFIG_PATH) {
    $cfg = Get-Content $CONFIG_PATH -Raw | ConvertFrom-Json
    $cfg.data_dir = $DATA_DIR
    $cfg | ConvertTo-Json -Depth 10 | Set-Content -Encoding UTF8 $CONFIG_PATH
    Write-Host "[wipe] patched $CONFIG_PATH data_dir -> $DATA_DIR"
}

Write-Host "[run] starting $NODE_EXE start --config $CONFIG_PATH"
& $NODE_EXE start --config $CONFIG_PATH
