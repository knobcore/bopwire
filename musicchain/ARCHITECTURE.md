...
### Mini-nodes — `musicchain-mini-node`
...
(`musicchain_player/lib/src/services/rats_client.dart:179-198`).
...
2. **librats DHT** — every successful `fingerprint.submit` triggers
   ...
   probing the same 20-byte SHA-1 key (`musicchain_player/lib/src/services/rats_client.dart:683-760`).
...
`stream.open` handlers (`musicchain_player/lib/src/services/player_server.dart:108-114`).
...
- No HTTP/3, no HTTP, no JSON-RPC-over-WebSocket. Every verb arrives as a
  librats typed message of type `musicchain.request`; every reply leaves as
  `musicchain.reply`.
...
- **librats TCP** — ephemeral port the OS picks (33000–34999 random by default,
  `musicchain_player/lib/src/services/rats_client.dart:96`). The DHT runs on a
...
- **Resumable download (multi-peer)** — `audio.piece_get(content_hash, offset,
  ...
  (`musicchain_player/lib/src/services/player_server.dart:116-160`). The
  ...
  exactly (`musicchain_player/lib/src/services/piece_downloader.dart:482-549`).
- **Streaming download (continuous)** — `stream.open(content_hash)` peer-to-peer
  ...
  (`musicchain_player/lib/src/services/player_server.dart:175-294`). Crucially,
...
- **DHT** — `rats_find_peers(sha1Hex)` runs over librats's BEP-5-compatible
  DHT. Full nodes announce themselves on `sha1("musicchain-fullnode-mainnet")`
  ...
  result set (`musicchain_player/lib/src/services/rats_client.dart:663-760`),
...
- **`routes.get`** — player → mini-node. Returns the full node routing table
  ...
  `musicchain_player/lib/src/services/rats_client.dart:1276-1288`.
- **`mininodes.list`** — player → mini-node. Returns every fellow mini-node
  ...
  via SharedPreferences (`musicchain_player/lib/src/services/rats_client.dart:478-527`).
...
- **Cellular** — symmetric NAT per [project-cellular-symmetric-nat]. Direct
  ...
  (`musicchain_player/lib/src/services/rats_client.dart:849-880`). Binary
...
- **Cone NAT (home Wi-Fi)** — possibly direct. After the mini-node observes
  ...
  (`musicchain_player/lib/src/services/rats_client.dart:600-623`), the
  player can attempt `rats_connect(public_address)` against another player's
  observed address. Per [project-musicchain-platforms] this is the desktop
  fallback path.
- **Open / port-forwarded** — direct. The player observes its own peer
  ...
  the relay-via mapping (`musicchain_player/lib/src/services/rats_client.dart:200-214`).
...
- **librats is FROZEN at v0.2.0** (commit `246557c`) per
  ...
  `RatsClient` (`musicchain_player/lib/src/services/rats_client.dart`). The
...
- **Phone Doze prompt** — first launch shows the
  ...
  (`musicchain_player/android/app/src/main/AndroidManifest.xml:27`). Without
...
- **20 s discovery refresh + bidirectional `mini.ping`** fills the cellular-
  ...
  (`musicchain_player/lib/src/services/librats_discovery.dart:62-64`); the
...
- **Multi-VPS topology** per [project-musicchain-multi-vps-topology]. Do not
  ...
  (`musicchain_player/lib/src/services/rats_client.dart:432-449`,
  `:537-567`).
