/**
 * node_main.cpp - Entry point for bopwire-node
 *
 * Usage:
...
static int cmd_status(const std::vector<std::string>& /*args*/) {
    ...
    std::cout << "bopwire-node status: use 'curl http://localhost:9334/api/v1/status'\n";
    return 0;
}
...
static int cmd_peers(const std::vector<std::string>& /*args*/) {
    std::cout << "bopwire-node peers: use 'curl http://localhost:9334/api/v1/peers'\n";
    return 0;
}
