...
/// Requests arrive on message type `bopwire.request` as JSON
/// `{req_id, type, body}`; replies go back on `bopwire.reply` as
...
private:
    ...
    /// given peer as individual `bopwire.mod` messages. Called when a
