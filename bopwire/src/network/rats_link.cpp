...
bool RatsLink::start() {
    ...
    // distribution is handled by BlockPropagator over bopwire.request.
...
// Populate bootstrap_vps_ from the env var BOPWIRE_VPS_BOOTSTRAP, which
...
void RatsLink::load_bootstrap_list() {
    ...
    const char* env = std::getenv("BOPWIRE_VPS_BOOTSTRAP");
    if (env && *env) {
...
// BlockPropagator (INV/getdata over bopwire.request + per-block DHT
