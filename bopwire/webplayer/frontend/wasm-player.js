...
(() => {
  ...
  class WasmPlayer {
    constructor() {
      this.ctx = null; this.gain = null; this.decoder = null;
      this.url = ''; this.totalBytes = 0; this.durationSec = 0;
    ...
    // the caller falls back to <audio>. durationSec drives the seek bar + seeking.
    ...
    async load(url, durationSec) {
      await this.stop();
      this.url = url; this.durationSec = durationSec || 0; this.totalBytes = 0;
      this.unlock();                      // reuse the gesture-unlocked context
      ...
      if (!make) { try { resp.body.cancel(); } catch (_) {} await this.stop(); throw new Error('unsupported:' + ct); }

      this.decoder = make();
      await this.decoder.ready;
      this._readTotal(resp, 0);
      this._pump(resp.body.getReader(), gen);
      this._tick();
    }
    ...
    async _pump(reader, gen) {
      try {
        while (this.playing && gen === this._gen) {
          ...
          let out;
          try { out = await this.decoder.decode(value); } catch (_) { continue; }
    ...
    async seek(posSec) {
      if (!this.ctx || !this.decoder || !this.totalBytes || !this.durationSec) return;
      ...
      if (this.decoder.reset) { try { await this.decoder.reset(); } catch (_) {} }
      this._abort = new AbortController();
    ...
    async stop() {
      ...
      for (const s of this._sources) { try { s.stop(); } catch (_) {} }
      this._sources = [];
      if (this.decoder && this.decoder.free) { try { this.decoder.free(); } catch (_) {} }
      this.decoder = null;
      // keep this.ctx / this.gain alive for reuse (recreating needs a gesture)
      this._next = 0; this._baseCtx = 0; this._basePos = 0; this.totalBytes = 0;
    }
  }
