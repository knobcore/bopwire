...
# (No gnome-keyring / D-Bus bundling: the wallet secret now lives in our own
# password-encrypted vault (SecureStore → native scrypt+AES-256-GCM keystore),
# not libsecret/Secret Service, so no keyring daemon is needed.)
